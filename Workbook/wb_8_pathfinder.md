...
## 2026-08-05 实车验证结果（里程碑）

- **稳定运行 20 分钟无中断**（网络问题确认是校园网环境，非程序；后续自建 AP 解决）
- **建图效果明显改善**：墙端完整（不再“短一截”），障碍能进图，D* 绕行正常
- **单车（Binah）基本完成** ✅

**验证结论**：根因 #1 修复（射线截断 + hit 优先 + 不对称参数）实车有效。

**剩余待办**（非阻塞）：#2 mark_obstacle 写 grid、#3 近场盲区、#4 map_delta 节流 / slam_task 事件驱动。
