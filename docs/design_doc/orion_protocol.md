...
> `Circle`（Task 18）：半径写死 0.5m（= 与圆心隔 1 格），车在环上均匀铺开，见 multi_robot_control.md §4.3。
...
| `> 1` | **群发任务**（每车自算散布） | 取 missions 中**第一个 `Goto` 或 `Circle`** 为目标点，members 名单随任务下发；每车确认自己在名单内 → 确定性分配自算散布位置（见 multi_robot_control.md §4），不在名单 → 忽略 |
