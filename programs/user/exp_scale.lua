-- Presented by KeJi
-- Date: 2026-06-02
-- 实验 E1: Pipeline Scaling
-- 用法: exec exp_scale model=xxx.pgguf nodes=N tokens=M prompt=...
-- 输出: 计时数据，用于 Scaling 曲线

COMMAND = "exp_scale"
DESCRIPTION = "E1 Scaling 实验: 测量不同节点数下的吞吐"
...
function execute(params)
    ...
    caps.print(string.format("EXP_SCALE model=%s nodes=%d max_tokens=%d", model, nodes, max_tokens))
    ...
    -- 4. 建立 tensor stream (第一个/最后一个远程节点)
    ...
    if not fwd_peer or not bwd_peer then caps.print("EXP_ERROR: no remote peers"); handle:release(); return end
