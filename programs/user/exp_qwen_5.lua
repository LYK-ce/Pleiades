-- Presented by KeJi
-- Date: 2026-06-02
-- 实验: 五节点流水线 (Qwen3-235B)
-- 用法: exec exp_qwen_5 model=xxx.pgguf tokens=128

COMMAND = "exp_qwen_5"
DESCRIPTION = "五节点流水线实验"
...
function execute(params)
    ...
    if #all_peers < 5 then
        caps.print(string.format("EXP_ERROR: need 5 nodes, only %d", #all_peers)); handle:release(); return
    end
    table.sort(all_peers, function(a, b) return a.layer_start < b.layer_start end)
    local peers = {all_peers[1], all_peers[2], all_peers[3], all_peers[4], all_peers[5]}

    caps.print(string.format("EXP_QWEN_5 model=%s nodes=5 tokens=%d", model, max_tokens))
    ...
    if tok ~= eos then sess:decode(tok) end
    ...
    caps.print("")
    caps.print("==== EXP_QWEN_5 RESULT ====")
    caps.print(string.format("NODES:5"))
