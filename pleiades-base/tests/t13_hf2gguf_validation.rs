...
fn t13_l1_gguf_analyze() {
    ...
    let arch = pleiades_base::GGUF_Analyze(&path).expect("GGUF_Analyze failed");
...
fn t13_l2_load_model() {
    ...
    let model = pleiades_base::GGUF_Load_Model(0, 999999, &path, &device)
    ...
    // 释放模型
    pleiades_base::GGUF_Unload_Model(model);
    println!("✅ L2 PASSED");
}
...
fn t13_l3_inference() {
    ...
    let input_text = "你好";
    let messages = vec![pleiades_base::Message {
        role: "user".to_string(),
