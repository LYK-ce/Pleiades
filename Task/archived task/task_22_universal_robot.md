...
> ⚠️ **【已废弃 2026-08-29】** 本方案「三层统一 + `MotionDevice` 统一 + Lua 决策」的思路不对（本质是「一份代码适配所有设备」的固化思维），已被 `task_23_universal_robot` 取代。新架构：**底座共享 + 设备端独有**，详见 `docs/heterogeneity_analysis.md`。
