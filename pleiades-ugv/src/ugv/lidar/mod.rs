...
/// 激光雷达扫描状态（Task 23 B2：从 core/state.rs 摘出，LidarDevice 内部维护）
...
use pleiades_base::robot::util::serial::port;
use crate::ugv::slam::task::spawn_slam;
...
pub struct LidarDevice {
    ...
    /// 内部维护的扫描状态（Task 23 B2：不再共享给 base）
...
impl LidarDevice {
    ...
    pub fn spawn(
        ...
        let state = Arc::new(RwLock::new(LidarState::default()));
        ...
        Ok(Self { serial_cmd_tx, state, cancel })
    }

    /// 读取最新扫描快照（Task 23 B2：急停等消费方从 LidarDevice 句柄获取）
    pub async fn get_scan(&self) -> Option<LaserScan> {
        self.state.read().await.scan.clone()
    }
...
impl LidarDevice {
    ...
    pub fn spawn_mock(
        ...
        (Self { serial_cmd_tx, state, cancel }, handle)
    }
}
